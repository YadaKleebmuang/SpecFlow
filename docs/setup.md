...
# 🛠️ คู่มือการติดตั้งระบบ SpecFlow (Python 3.10 & Rasa)

คู่มือฉบับนี้สรุปขั้นตอนทั้งหมดตั้งแต่การโคลนโปรเจกต์ใหม่ ไปจนถึงการเปิดระบบเพื่อพิมพ์คุยกับแชทบอท โดยเจาะจงใช้งานร่วมกับ **Python 3.10** ซึ่งเป็นเวอร์ชันที่เสถียรที่สุดสำหรับ Rasa 3.x

---

## ⚙️ ขั้นตอนการติดตั้งและเริ่มต้นระบบ

### 1️⃣ ขั้นที่ 1: ตรวจสอบและสร้างสภาพแวดล้อมจำลอง (venv)
ในระบบ Windows หากมี Python หลายเวอร์ชัน ให้ระบุเวอร์ชัน `3.10` ในการสร้างโฟลเดอร์ `venv`:
```bash
# รันคำสั่งนี้ที่โฟลเดอร์หลักของโปรเจกต์ (เช่น SpecFlow หรือ SpecFlow-main)
py -3.10 -m venv venv
```
> [!NOTE]
> คำสั่งนี้จะสร้างโฟลเดอร์จำลองชื่อ `venv` ขึ้นมาในโปรเจกต์ของคุณ

---

### 2️⃣ ขั้นที่ 2: การเปิดใช้งาน (Activate) Virtual Environment
เลือกใช้คำสั่งตามเครื่องมือ (Terminal) ที่คุณกำลังเปิดใช้งานอยู่:

* **สำหรับ Git Bash (แนะนำ):**
  ```bash
  source venv/Scripts/activate
  ```
...
> [!IMPORTANT]
> หากสำเร็จ หน้าต่างคอมมานด์ไลน์ของคุณจะมีสัญลักษณ์ **`(venv)`** ปรากฏอยู่ที่ต้นบรรทัดคำสั่งเสมอ

---

### 3️⃣ ขั้นที่ 3: ตรวจสอบเวอร์ชันภายในระบบจำลอง
ตรวจสอบความถูกต้องเพื่อให้มั่นใจว่า Python ในระบบใช้เวอร์ชัน `3.10.x` แล้ว:
```bash
python --version
```
...
### 4️⃣ ขั้นที่ 4: การติดตั้ง Dependencies และไลบรารี
ดำเนินการติดตั้งไลบรารีที่โปรเจกต์ต้องใช้งาน:
```bash
...
```
> [!WARNING]
> ห้ามกดยกเลิกกลางคัน (ห้ามกด Ctrl+C) จนกว่าระบบจะขึ้นแสดงความสำเร็จว่า `Successfully installed ...` ทั้งหมด เนื่องจากตัวโปรแกรมมีขนาดใหญ่และใช้เวลาประมาณ 3-5 นาที

---

### 5️⃣ ขั้นที่ 5: การเทรนโมเดลแชทบอท (Rasa Train)
นำเข้าชุดข้อมูลการสนทนาภาษาไทยเพื่อนำมาเทรนให้แชทบอทเข้าใจตรรกะการสนทนา:
```bash
# 1. เข้าไปที่โฟลเดอร์ย่อย
cd app/rasa

# 2. สั่งเทรนโมเดล (บอทจะถูกเซฟอัตโนมัติในโฟลเดอร์ models/)
rasa train

# 3. ถอยกลับมายังโฟลเดอร์หลักของโปรเจกต์
cd ../..
```

---

### 6️⃣ ขั้นที่ 6: รันระบบเซิร์ฟเวอร์ SpecFlow
เริ่มต้นการทำงานของ Rasa Server, Action Server และ Ngrok พร้อมกันผ่านสคริปต์หลัก:
```bash
python run.py
```
*(เมื่อคุณต้องการปิดหรือหยุดการรันระบบทั้งหมด ให้กดปุ่ม **`Ctrl + C`** ที่หน้าต่างรันคำสั่งนี้ได้เลยค่ะ)*
